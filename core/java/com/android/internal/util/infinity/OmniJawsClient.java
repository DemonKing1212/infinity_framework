...
public class OmniJawsClient {
    ...
    public static final Uri CONTROL_URI = Uri.parse("content://org.omnirom.omnijaws.provider/control");
    ...
    private final List<OmniJawsObserver> mObserver = new ArrayList<>();
    ...
    public void queryWeather() {
        ...
        try (Cursor weatherCursor = mContext.getContentResolver().query(WEATHER_URI, WEATHER_PROJECTION, null, null, null)) {
            if (weatherCursor != null && weatherCursor.moveToFirst()) {
                mCachedInfo = new WeatherInfo();
                List<DayForecast> forecastList = new ArrayList<>();

                do {
                    if (weatherCursor.getPosition() == 0) {
                        ...
                        mCachedInfo.timeStamp = Long.valueOf(weatherCursor.getString(11));
        ...
        try (Cursor settingsCursor = mContext.getContentResolver().query(SETTINGS_URI, SETTINGS_PROJECTION, null, null, null)) {
    ...
    private Drawable getDefaultConditionImage() {
        ...
        try {
            ...
            Log.e(TAG, "getDefaultConditionImage", e);
        }
    ...
    public boolean isOmniJawsEnabled() {
        ...
        try (Cursor c = mContext.getContentResolver().query(SETTINGS_URI, SETTINGS_PROJECTION, null, null, null)) {
    ...
    public void removeObserver(OmniJawsObserver observer) {
        if (mObserver.remove(observer) && mObserver.isEmpty()) {
            unregisterReceiver();
        }
    }
    ...
    private void registerReceiverIfNeeded() {
        ...
        if (mObserver.size() == 1 && !mWeatherReceiverRegistered) {
            if (mReceiver != null && mWeatherReceiverRegistered) {
                try {
                    mContext.unregisterReceiver(mReceiver);
                } catch (Exception e) {
                    Log.e(TAG, "registerReceiverIfNeeded - unregister failed", e);
                }
            }
