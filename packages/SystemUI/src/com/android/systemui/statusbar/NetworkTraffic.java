...
public class NetworkTraffic extends TextView implements TunerService.Tunable,
    ...
    private final BroadcastReceiver mThemeReceiver;
    ...
    public NetworkTraffic(Context context, AttributeSet attrs, int defStyle) {
        ...
        mTrafficHandler = new Handler(context.getMainLooper()) {
            @Override
    ...
    protected void onAttachedToWindow() {
        ...
        if (!mAttached) {
            ...
            mContext.registerReceiver(mThemeReceiver, new IntentFilter(Intent.ACTION_CONFIGURATION_CHANGED));
    ...
    public void onDarkChanged(ArrayList<Rect> areas, float darkIntensity, int tint) {
        if (mColorIsStatic) return;
        setTint(DarkIconDispatcher.getTint(areas, this, tint));
    }
	...
	switch (state.visible ? STATE_ICON : STATE_HIDDEN) {
	    case STATE_ICON:
		setVisibleState(STATE_ICON, false);
		break;
	    case STATE_DOT:
		mShowDot = true;
		setVisibleState(STATE_DOT, false);
		break;
            case STATE_HIDDEN:
		setVisibleState(STATE_HIDDEN, false);
		break;
	}
        updateVisibility();
    }
    ...
    public void onTuningChanged(String key, String newValue) {
        switch (key) {
            ...
            case NETWORK_TRAFFIC_AUTOHIDE_THRESHOLD:
                ...
                mAutoHideThreshold = autohidethreshold * Kilo;
                updateViews();
                break;
            ...
            case NETWORK_TRAFFIC_HIDEARROW:
                ...
                setGravity(mHideArrows ? Gravity.CENTER : Gravity.END|Gravity.CENTER_VERTICAL);
                setTrafficDrawable();
                break;
        }
    }
    ...
    private void displayStatsAndReschedule() {
        final boolean showUpstream = mMode == MODE_UPSTREAM_ONLY || mMode == MODE_UPSTREAM_AND_DOWNSTREAM;
        final boolean showDownstream = mMode == MODE_DOWNSTREAM_ONLY || mMode == MODE_UPSTREAM_AND_DOWNSTREAM;
        ...
        if (getVisibility() == View.VISIBLE && (mSubMode != submode || mTrafficActive != trafficactive)) {
            mSubMode = submode;
    ...
    private void setTrafficDrawable() {
        ...
        Drawable drawable = drawableResId != 0 ? getContext().getDrawable(drawableResId) : null;
    ...
    private void updateVisibility() {
        ...
        switch (mVisibleState) {
            case STATE_ICON:
                visible = mEnabled && mIsActive && !TextUtils.isEmpty(getText()) 
                        && !mKeyguardShowing && mSystemIconVisible;
                break;
            case STATE_DOT:
                visible = mEnabled && !mKeyguardShowing && mShowDot;
                break;
            case STATE_HIDDEN:
            default:
                visible = false;
