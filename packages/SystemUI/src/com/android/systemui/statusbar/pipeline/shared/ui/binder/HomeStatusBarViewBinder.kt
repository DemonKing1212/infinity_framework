...
constructor(
                ...
                if (StatusBarRootModernization.isEnabled) {
                    ...
                    launch {
                        ...
                        clockState.collect { state ->
                            ...
                            val chipNeedsUpdate = (lastChipStyle != state.showChip)
                                        || (lastClockPosition != state.position)
                            if (chipNeedsUpdate) {
                                applyClockChip(
                                    context = context,
                                    chipStyle = state.showChip,
    ...
    private fun applyClockChip(
        ...
        fun reset(clock: Clock, padding: Padding) {
            ...
            clock.textAlignment = View.TEXT_ALIGNMENT_VIEW_START
        }
        ...
        val chipTopBottomPadding = context.resources.getDimensionPixelSize(R.dimen.status_bar_clock_chip_tb_padding)
        val chipLeftRightPadding = context.resources.getDimensionPixelSize(R.dimen.status_bar_clock_chip_lr_padding)
        
        val backgroundDrawable = if (chipStyle in 1..12) chipBackgrounds[chipStyle - 1] else chipBackgrounds[0]

        activeClock.setBackgroundResource(backgroundDrawable)
        activeClock.setPaddingRelative(chipLeftRightPadding, chipTopBottomPadding, chipLeftRightPadding, chipTopBottomPadding)
        activeClock.textAlignment = View.TEXT_ALIGNMENT_CENTER
    }
