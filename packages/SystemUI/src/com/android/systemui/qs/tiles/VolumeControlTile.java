...
public class VolumeControlTile extends QSTileImpl<BooleanState> 
    ...
    private static final String QS_REFACTOR_ENABLED = "qs_refactor_enabled";
    ...
    protected void handleUpdateState(BooleanState state, Object arg) {
        if (isRefactorEnabled()) {
            ...
            state.label = mContext.getString(R.string.quick_settings_volume_tile_label);
            state.secondaryLabel = mContext.getString(R.string.stream_unavailable_by_unknown);
        ...
        if (volumePercent <= 1) {
            state.icon = ResourceIcon.get(R.drawable.ic_volume_media_mute);
            state.secondaryLabel = mContext.getString(R.string.quick_settings_volume_tile_muted);
        } else if (volumePercent > 0 && volumePercent < 50) {
            state.icon = ResourceIcon.get(R.drawable.ic_volume_media_low);
            state.secondaryLabel = mContext.getString(R.string.quick_settings_volume_tile_low);
        } else {
            state.icon = ResourceIcon.get(R.drawable.ic_volume_media);
            state.secondaryLabel = mContext.getString(R.string.quick_settings_volume_tile_high);
        }
    }
}
